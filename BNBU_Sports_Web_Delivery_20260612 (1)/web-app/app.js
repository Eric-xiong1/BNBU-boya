...
function apiHealthUrl() {
  const base = String(state.apiBaseUrl || "").trim().replace(/\/+$/, "");
...
function getApiBase() { return state.apiBaseUrl || window.location.origin; }
...
function courseRoster(courseId = state.courseId) {
  if (!state.rosters[courseId]) state.rosters[courseId] = seedRoster(courseId);
...
function submitDeliveryForCurrentCourse() {
  ...
  setTeacherNotice(blocked ? `${courseOfferingLabel(course)} 预检已提交给体育部，但仍有 ${count} 项问题。` : `${courseOfferingLabel(course)} 预检已通过并提交给体育部确认归档。`);
}
  ...
  app.innerHTML = `
    <main class="login-screen">
      <section class="login-brand">
        ...
        <p>面向 BNBU 体育部的 Web 工作台。先让老师少干活，再把打卡、考试、签到、体测和导出变成一条可靠流程。</p>
      </section>
      <section class="login-panel">
        <div class="login-card">
          ...
          <form data-action="login">
            ...
            ${state.loginError ? `<div class="login-error"><p>${escapeHtml(state.loginError)}</p></div>` : ""}
            <div class="button-row" style="margin-top:18px">
              <button class="primary-button" type="submit" ${state.loginLoading ? "disabled" : ""}>${state.loginLoading ? '登录中…' : '进入系统'}</button>
              <button class="ghost-button" type="button" data-action="quick-teacher">老师端演示</button>
              <button class="ghost-button" type="button" data-action="quick-admin">管理端演示</button>
              <button class="ghost-button" type="button" data-action="quick-manager">负责人演示</button>
            </div>
          </form>
        </div>
      </section>
...
function renderLayout(content) {
  ...
  const meta = state.role === "teacher" ? `${course.semester} / ${courseOfferingLabel(course)}` : "BNBU SPORTS DEPARTMENT";
    ...
    <div class="app-shell">
      <aside class="sidebar">
        <div class="brand-block">
          ${renderLogo()}
          <div>
            <p class="eyebrow">BNBU</p>
            <h1>体育成绩<br />工作台</h1>
          </div>
        </div>
        <nav class="nav-list">
          <div class="nav-section">${state.role === "teacher" ? "Teacher" : state.role === "admin" ? "Admin" : "Manager"}</div>
          ${navForRole()
      ...
      <main class="main-area">
        <header class="topbar">
          ...
          <div class="top-actions">
            <button type="button" class="icon-button" aria-label="搜索">⌕</button>
            ...
            .map(
              (item) => `
                <tr class="${item.id === state.activeReviewId ? "is-hot" : ""}" data-review="${item.id}">
                  <td><strong>${item.name}</strong><br /><small>${item.studentId}</small></td>
                  <td>${item.type}</td>
                  <td>${item.hours}h</td>
                  <td>${item.risk}</td>
...
function renderSelectedReviewSummary() {
  ...
  return `
    <div class="panel-body selected-record">
      <div>
        <span class="status ${statusClass(active.status)}">${active.status}</span>
        <h3>${active.name}</h3>
        <p class="muted">${active.studentId} / ${active.type} / ${active.hours}h</p>
      </div>
      <dl>
        <dt>提交任务</dt>
        <dd>${active.task}</dd>
    ...
    <div class="${compact ? "two-col" : "content-grid"}" style="${compact ? "grid-template-columns:minmax(0,1fr) 330px;gap:0" : ""}">
      ...
      <aside class="proof-card">
        <div class="proof-media">
          ${(active.proofFiles && active.proofFiles.length > 0) ? active.proofFiles.map((url, i) => `
            <a href="${escapeHtml(url)}" target="_blank" class="proof-thumb" title="查看大图">
        ...
        <div class="proof-meta"><small>${active.proofFiles ? active.proofFiles.length + ' 张凭证' : '0 张凭证'} · ${active.task}</small></div>
        <dl>
          <dt>提交任务</dt><dd>${active.task}</dd>
          <dt>异常原因</dt><dd>${active.reason}</dd>
          <dt>审核记录</dt><dd>${active.comment || "尚未填写"}</dd>
        </dl>
        <div class="form-grid review-form">
          ...
          <div class="field"><label for="reviewComment">审核备注</label><input id="reviewComment" data-review-comment value="${active.comment || ""}" placeholder="填写驳回、补材料或通过说明" /></div>
        </div>
        <div class="button-row">
            ...
            .map(
              (student) => `
                <tr>
                  ...
                  <td><span class="status ${statusClass(student.status)}">${student.status}</span></td>
                </tr>
              `,
            )
            .join("")}
...
function confirmImportPreview() {
  ...
  setTeacherNotice(`${courseOfferingLabel()} 已导入 ${imported.length} 名有效学生，${rejected.length} 行未通过校验。`);
}
  ...
  return panel("学生名单", "P0-06", `
    <div class="panel-body">
      <div class="form-grid" style="margin-bottom:16px">
        <div class="field"><label for="studentSearch">搜索学生</label><input id="studentSearch" data-student-search placeholder="姓名 / 学号" value="${state.studentSearch}" /></div>
        <div class="field">
          ...
          .map(
            (student) => `
              <tr>
                <td><strong>${student.name}</strong><br /><small>${student.id}</small></td>
                ${items
                  .map((_, itemIndex) => `<td><input class="input-cell" name="exam_${student.id}_${itemIndex}" type="number" min="0" max="100" value="${student.score.examItems[itemIndex]}" /></td>`)
                  .join("")}
                <td><strong>${student.exam}</strong></td>
                <td><span class="status ${statusClass(student.score.examStatus)}">${student.score.examStatus}</span></td>
              </tr>
            `,
          )
          .join("")}
        ...
        ${studentsForCourse()
          .map(
            (student) => `
              <tr>
                <td><strong>${student.name}</strong><br /><small>${student.id}</small></td>
                <td><select name="att_${student.id}_week6">${attendanceOptions(student.score.attendanceWeeks.week6)}</select></td>
                <td><select name="att_${student.id}_week7">${attendanceOptions(student.score.attendanceWeeks.week7)}</select></td>
                <td><input class="input-cell" name="att_${student.id}_score" type="number" min="0" max="100" value="${student.attendance}" /></td>
                <td><span class="status ${statusClass(student.score.attendanceStatus)}">${student.score.attendanceStatus}</span></td>
              </tr>
            `,
          )
          .join("")}
        ...
        ${students.map((student) => {
          ...
          return `
              <tr>
                ...
                <td>${genderLabel}</td>
                <td>${gradeLabel}</td>
                <td><input class="input-cell" name="phys_${student.id}_endurance" value="${student.score.physicalItems.endurance}" style="width:80px" data-phys-endurance data-student-id="${student.id}" data-gender="${student.gender || ''}" data-grade="${student.gradeLevel || ''}" placeholder="如 3'30"" /></td>
                <td><input class="input-cell" name="phys_${student.id}_sprint" value="${student.score.physicalItems.sprint}" /></td>
                <td><input class="input-cell" name="phys_${student.id}_jump" value="${student.score.physicalItems.jump}" /></td>
                <td><button type="button" class="ghost-button" data-auto-convert data-student-id="${student.id}" style="font-size:11px;padding:2px 6px">换算</button><br /><small id="convertResult_${student.id}" class="status"></small></td>
                <td><input class="input-cell" name="phys_${student.id}_score" id="phys_score_${student.id}" type="number" min="0" max="100" value="${student.physical}" /></td>
              </tr>
            `;
          }).join("")}
      </tbody></table></div>
...
function renderGrades() {
  const weights = gradeWeights();
  const targets = sportRuleTargets();
  const weightText = `当前权重：体育打卡 ${Math.round(weights.checkin * 100)}% / 专项 ${Math.round(weights.exam * 100)}% / 平时 ${Math.round(weights.attendance * 100)}% / 体测 ${Math.round(weights.physical * 100)}%`;
    ...
    <div class="table-wrap"><table><thead><tr><th>学生</th><th>体育打卡</th><th>专项</th><th>平时</th><th>体测</th><th>总分</th><th>来源追溯</th><th>缺失项</th></tr></thead><tbody>
      ${studentsForCourse()
        .map((student) => {
          ...
          const checkinComplete = student.course >= targets.courseRequired && student.general >= targets.generalRequired;
          return `<tr><td><strong>${student.name}</strong><br /><small>${student.id}</small></td><td>${breakdown.checkinScore}<br /><small>计 ${breakdown.checkinWeighted}${student.organizationCredit ? ` / ${escapeHtml(student.organizationCredit.organization)} 抵扣` : ""}</small></td><td>${student.exam}<br /><small>计 ${breakdown.examWeighted}</small></td><td>${student.attendance}<br /><small>计 ${breakdown.attendanceWeighted}</small></td><td>${student.physical}<br /><small>计 ${breakdown.physicalWeighted}</small></td><td><strong>${breakdown.total}</strong></td><td><small>名单：${escapeHtml(sources.roster)}<br />打卡：${escapeHtml(sources.checkin)}<br />专项：${escapeHtml(sources.exam)} / 平时：${escapeHtml(sources.attendance)} / 体测：${escapeHtml(sources.physical)}</small></td><td><span class="status ${checkinComplete ? "ok" : "warn"}">${checkinComplete ? "无" : "打卡未满"}</span></td></tr>`;
        })
        .join("")}
    </tbody></table></div>
  `);
...
function integrationSnapshot() {
  ...
  return {
    ...
    visualTheme: {
      primary: "#3A9DF6",
      secondary: "#7EBEFB",
      cssFile: "web-app/styles-campus-blue.css",
    },
    qualityGates: qualityGateGroups(),
    browserCapabilities: browserCapabilityRows(),
    apiRequestPolicy,
    storageKey,
    apiContract: "backend-handoff/openapi.yaml",
    roles: [
...
function renderAdminApiHandoff() {
  ...
  return `
    ...
    ${panel("后端交付状态", "BACKEND HANDOFF", `
      <div class="panel-body">
        <div class="notice">当前 Web 已完成老师端、管理端、组织负责人端的前端闭环。后端接入时以 <strong>backend-handoff/openapi.yaml</strong> 为接口草案，以导出的状态快照核对字段。稳定性目标按 <strong>200 名 Web 用户同时登录操作</strong> 做后端联调压测。</div>
        <div class="button-row" style="margin-top:16px">
...
function renderConversionRules() {
  return adminFrame(
    panel("体测换算表", "P1-07", `
      <div class="table-wrap">
        <table>
          <thead><tr><th>项目</th><th>指标</th><th>换算方法</th><th>版本</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
            ${state.admin.conversionRules
              .map(
                (rule) => `
                  <tr>
                    <td><strong>${escapeHtml(rule.item)}</strong></td>
                    <td>${escapeHtml(rule.metric)}</td>
                    <td>${escapeHtml(rule.method)}</td>
                    <td>${escapeHtml(rule.version)}</td>
                    <td><span class="status ${statusClass(rule.status)}">${escapeHtml(rule.status)}</span></td>
                    <td><button class="danger-button" data-admin-delete="conversionRules" data-admin-id="${rule.id}">删除</button></td>
                  </tr>
                `,
              )
              .join("")}
    ...
    `),
    panel("新增换算项", "CONVERSION FORM", `
      <form class="panel-body form-grid" data-admin-form="conversion">
        <div class="field"><label for="conversionItem">项目</label><input id="conversionItem" name="item" value="引体向上 / 仰卧起坐" /></div>
        <div class="field"><label for="conversionMetric">指标</label><input id="conversionMetric" name="metric" value="次数" /></div>
        <div class="field"><label for="conversionMethod">换算方法</label><input id="conversionMethod" name="method" value="分段计分" /></div>
        <div class="field"><label for="conversionVersion">版本</label><input id="conversionVersion" name="version" value="CN-2026-v1" /></div>
        <button class="primary-button" type="submit">保存换算项</button>
      </form>
    `),
  );
}
          ...
          <tbody>
            ${items.map((r) => `
              <tr>
                ...
                <td>${(r.proofFiles && r.proofFiles.length > 0) ? `<a href="${escapeHtml(r.proofFiles[0])}" target="_blank" class="ghost-button" style="font-size:11px;padding:2px 6px">查看(${r.proofFiles.length})</a>` : '-'}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
...
function renderConversionDetail() {
  const tab = state.conversionTab || 'freshman_sophomore-male';
  const [gradeGroup, gender] = tab.split('-');
  const genderLabel = gender === 'male' ? '男' : '女';
  const itemLabel = gender === 'male' ? '1000米' : '800米';
  const gradeLabel = gradeGroup === 'freshman_sophomore' ? '大一/大二' : '大三/大四';
  ...
  if (state.conversionTableData && state.conversionTableData.gradeGroup === gradeGroup && state.conversionTableData.gender === gender) {
    ...
    const rulesKey = `${gender === 'male' ? 'male' : 'female'}-${gradeGroup}`;
    const rulesRef = {
    ...
    const rawTable = rulesRef[rulesKey] || [];
    for (const [score, maxSeconds] of rawTable) {
      const mins = Math.floor(maxSeconds / 60);
      const secs = maxSeconds % 60;
      const rawVal = `${mins}'${String(secs).padStart(2,'0')}"`;
      entries.push({ raw_seconds: maxSeconds, raw_value: rawVal, converted_score: score });
    }
    ...
    panel("体测换算表 — 按秒计分", "P1-07 ENHANCED", `
      ...
      <div class="panel-body conversion-tabs">
        <div class="tab-bar" style="display:flex;gap:4px;margin-bottom:16px;flex-wrap:wrap">
          ${tabs.map(t => `
            <button class="ghost-button ${tab === t.key ? 'primary-button' : ''}" data-conversion-tab="${t.key}" style="font-size:12px;padding:4px 8px">${t.label}</button>
          `).join('')}
        </div>
      </div>
      <div class="table-wrap" style="max-height:500px;overflow-y:auto">
        <table>
          <thead><tr><th>成绩</th><th>秒数</th><th>分数</th><th>等级</th></tr></thead>
          <tbody>
            ${entries.map(e => {
              const tier = e.converted_score >= 90 ? '优秀' : e.converted_score >= 80 ? '良好' : e.converted_score >= 60 ? '及格' : '不及格';
              return `
                <tr>
                  <td>${e.raw_value}</td>
      ...
      <div class="panel-body">
        <p style="color:var(--color-muted);font-size:12px">${gradeLabel} · ${genderLabel} · ${itemLabel} — ${summaryText}。管理端可配置每1秒一个分数，系统自动检查空档。</p>
      </div>
    `)
  );
}
                ...
                <tbody>
                  ${pendingExemptions.map((e) => `
                    <tr>
                      ...
                      <td>${e.proofFiles && e.proofFiles.length > 0 ? `<a href="${escapeHtml(e.proofFiles[0])}" target="_blank" class="ghost-button" style="font-size:11px;padding:2px 6px">查看(${e.proofFiles.length})</a>` : '无凭证'}</td>
                      <td>
...
function renderRouteUnsafe() {
  ...
  let content = "";
  if (state.route === "teacher-dashboard") content = renderDashboard();
  ...
  else if (state.route === "teacher-students") content = renderStudents();
  else if (state.route === "teacher-progress") content = panel("打卡进度", "P0-07", renderProgressRows());
  else if (state.route === "teacher-review") content = renderReviewPage();
  else if (state.route === "teacher-exam") content = renderExam();
  else if (state.route === "teacher-attendance") content = renderAttendance();
  else if (state.route === "teacher-physical") content = renderPhysical();
  else if (state.route === "teacher-grades") content = renderGrades();
  else if (state.route === "teacher-export") content = renderExport();
  else if (state.route === "admin-dashboard") content = renderAdminDashboard();
  else if (state.route === "admin-overview") content = renderAdminOverview();
  ...
  else if (state.route === "admin-logs") content = renderAdminLogs();
...
function handleRosterFile(file) {
  ...
  reader.onload = () => {
    const rows = importRowsFromCsv(String(reader.result || ""));
    state.importPreview = buildImportPreview(rows);
    state.imported = true;
    const validCount = state.importPreview.filter((row) => row.valid).length;
    const invalidCount = state.importPreview.length - validCount;
    setTeacherNotice(`${file.name} 读取完成：${validCount} 行可导入，${invalidCount} 行需处理。`);
...
function handleManagerImportFile(file, type) {
  if (!file) return;
  const reader = new FileReader();
  reader.onload = () => {
    const rows = importMembershipRowsFromCsv(String(reader.result || ""), type);
    ...
    const invalidCount = nextPreview.length - validCount;
    setManagerNotice(`${file.name} 读取完成：${validCount} 行可导入，${invalidCount} 行需处理。`);
...
function confirmManagerImportPreview(type) {
  ...
  setManagerNotice(`${label}成员已导入 ${imported.length} 行，${rejected.length} 行未通过校验。`);
...
function handleAdminForm(form) {
  const data = new FormData(form);
  const type = form.dataset.adminForm;

  if (type === "semester") {
    state.admin.semesters.unshift({
      id: adminId("s"),
    ...
    setAdminNotice("学期已保存，可在列表中设为当前学期。");
  }

  if (type === "course") {
    state.admin.courses.unshift({
      id: adminId("ac"),
    ...
    setAdminNotice("课程已加入管理列表。");
  }

  if (type === "user") {
    state.admin.users.unshift({
      id: adminId("u"),
...
function handleScoreForm(form) {
  ...
  setTeacherNotice(`${courseOfferingLabel()} ${label}已保存，成绩汇总和导出数据已同步更新。`);
  renderRoute();
}
...
function handleDeliveryAction(courseId, action) {
  ...
  if (action === "return") {
    ...
    record.updatedAt = formatDateTime();
    setAdminNotice(`${courseOfferingLabel(course)} 已退回任课老师清理。`);
    logAction("体育部管理员", "退回成绩预检", `${courseOfferingLabel(course)} / ${record.comment}`);
  }

  if (action === "remind") {
    record.comment = count ? `已提醒老师处理 ${count} 项问题。` : "已提醒老师提交最终确认。";
    record.updatedAt = formatDateTime();
    setAdminNotice(`${courseOfferingLabel(course)} 提醒已记录。`);
    logAction("体育部管理员", "提醒老师处理预检", courseOfferingLabel(course));
  }
}
...
document.addEventListener("submit", (event) => {
  ...
  if (taskForm) {
    ...
    logAction("体育任课老师", "创建课程任务", `${courseOfferingLabel()} / ${data.get("title") || "未命名任务"}`);
...
document.addEventListener("input", (event) => {
  ...
  if (event.target.matches("[data-api-base-url]")) {
    state.apiBaseUrl = event.target.value.trim().replace(/[<>"'`\\\s]/g, "");
    saveState();
  }
...
document.addEventListener("click", (event) => {
  ...
  if (target.dataset.managerAction) {
    ...
    if (membership) {
      if (target.dataset.managerAction === "approve-member") {
        ...
        membership.status = "认证有效";
        membership.offset = canOffset ? "可抵扣" : "不抵扣";
        membership.comment = canOffset ? "负责人已确认身份，其他运动自动抵扣" : "负责人已确认身份，但组织类型不产生体育抵扣";
        membership.updatedBy = "组织负责人";
        membership.updatedAt = formatDateTime();
        logAction("组织负责人", "确认成员身份", `${membership.studentName} / ${membership.organization}`);
        setManagerNotice(`${membership.studentName} 的 ${membership.organization} 身份已确认，${membership.offset === "可抵扣" ? "其他运动已自动抵扣" : "不产生体育抵扣"}。`);
      } else {
        membership.status = "不通过";
        membership.offset = "不抵扣";
        membership.comment = "负责人驳回认证";
        membership.updatedBy = "组织负责人";
        membership.updatedAt = formatDateTime();
        logAction("组织负责人", "驳回成员认证", `${membership.studentName} / ${membership.organization}`);
        setManagerNotice(`${membership.studentName} 的 ${membership.organization} 认证已驳回。`);
      }
      renderRoute();
    }
    return;
  ...
  if (target.dataset.taskAction) {
    ...
    if (!task) return;
    handleTaskAction(task, target.dataset.taskAction);
    renderRoute();
    return;
  }

  if (target.dataset.deliveryAction) {
    handleDeliveryAction(target.dataset.courseId, target.dataset.deliveryAction);
    renderRoute();
    return;
  }

  if (target.dataset.adminCourseAction) {
    const course = courses.find((item) => item.id === target.dataset.courseId);
    if (!course) return;
  ...
  if (target.dataset.action === "confirm-manager-import") {
    confirmManagerImportPreview(target.dataset.importType || "team");
    renderRoute();
    return;
  }
  ...
  if (target.dataset.action === "submit-delivery") {
    submitDeliveryForCurrentCourse();
    renderRoute();
    return;
  }
