...
class ApiStudentRepository(
     ...
     * user. No fallback to local or mock data — the app shows errors and retry
     * affordances instead.
     */
    override suspend fun loadWorkspaceAsync(): StudentWorkspace = withContext(Dispatchers.IO) {
        ...
        val tasksResponse: StudentTaskListResponse = try {
        ...
        buildWorkspace(summary, records, memberships, notices, allTasks)
    }
    ...
    private fun buildWorkspace(
        ...
        // endpoints. Start with zeros — they will populate when the backend
        // adds score fields to the summary response.
        val grades = GradeRow(
            ...
            physical = 0,
            total = 0,
            sourceTrace = "API: summary endpoint — scores not yet provided",
